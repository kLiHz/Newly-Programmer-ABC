...
有些工作，通过命令行操作计算机的**效率可能会更高**；

由于只和字符打交道，应用程序的**开发成本可能会比较低**；

作为程序员，**可能经常会使用命令行**操作计算机。

但是请注意，**不要盲目崇拜命令行操作**。对于一些任务，可能有有更高效的操作方式。
...
接下来我们遇到的很多“程序”，都属于 CLI 程序，也就是说，只能进行文本输入输出的程序。他们从终端获取输入，将文本输出在终端上。

他们区别于图形界面（GUI，Graphics User Interface）程序，这些程序**通过图形化的界面**与用户交流——获取用户的输入以及输出信息给用户。

我们目前使用的基本都是图形化程序。**虚拟终端**就是一种图形化程序，它模拟传统的终端，与 CLI 程序交流。
...
这里的“`list`”“`--upgradable`”都是传递给“`apt`”程序的参数
...
![Windows的“运行”对话框](./assets/win-run-dialog.png)

![运行PowerShell的Win32控制台](./assets/powershell-in-win32-console.png)
...
**脱字符表示法**（Caret notation）是对 ASCII 码不可打印的控制字符的一种表示法。用一个脱字符（^）后跟一个大写字符来表示一个控制字符的ASCII码值。
