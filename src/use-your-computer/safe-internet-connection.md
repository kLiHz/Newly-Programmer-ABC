...
有些开发者、公司提供的软件、应用程序（比如浏览器、操作系统），出于个人或其他的目的，也会窃取用户的个人数据，上传到远程服务器，并提供（售卖）给对数据感兴趣的第三方。
...
- 直连即为不转发流量到代理服务器；
- 规则代理最为常用，其会根据事先编写好的“规则（rule）”将不同类型的流量转发到不同的代理服务器上，一些不需要代理的流量便不再进行代理；
- 全局代理则将所有流量都进行转发。
